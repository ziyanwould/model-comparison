...
app = FastAPI(
    ...
    version="0.2.1" # Version updated for model_name support
)
...
class AgentApiConfig(BaseModel):
    ...
    model_name: Optional[str] = None # For specifying model ID, e.g., gpt-4o-mini

class CustomConfigs(BaseModel):
    general_agent: Optional[AgentApiConfig] = None
    finetuned_agent: Optional[AgentApiConfig] = None # model_name might not be used by FineTunedAgent current API

class EvaluateRequest(BaseModel):
    custom_configs: Optional[CustomConfigs] = None

@app.post("/evaluate", summary="执行智能体评测", description="运行预定义的测试用例，对比不同智能体的表现。可以提供自定义的API配置，包括模型名称。")
async def evaluate_agents_endpoint(request_data: EvaluateRequest = EvaluateRequest()):
    ...
    if request_data and request_data.custom_configs:
        ...
        # custom_configs_dict = request_data.custom_configs.dict(exclude_none=True)
